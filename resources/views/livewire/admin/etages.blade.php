<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h2 class="text-2xl font-bold text-slate-900">Étages</h2>
            <p class="text-slate-500 mt-1">Étages des bâtiments.</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-4 mb-4 grid grid-cols-1 md:grid-cols-3 gap-3">
        <div class="md:col-span-2 relative">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Rechercher par nom..." class="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
        </div>
        <select wire:model.live="filterBatiment" class="px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
            <option value="">Tous les bâtiments</option>
            @foreach($batiments as $b)
                <option value="{{ $b->id }}">{{ $b->nom }} ({{ $b->etablissement?->nom }})</option>
            @endforeach
        </select>
    </div>
        ...
        <table class="w-full">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase">Bâtiment</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase">Services</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($etages as $e)
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3"><span class="inline-flex items-center justify-center w-9 h-9 bg-indigo-50 text-indigo-700 font-bold rounded-lg">{{ $e->numero }}</span></td>
                        <td class="px-4 py-3 font-medium text-slate-900">{{ $e->nom ?: '—' }}</td>
                        <td class="px-4 py-3 text-sm text-slate-600">
                            {{ $e->batiment?->nom }}
                            <span class="text-xs text-slate-400 block">{{ $e->batiment?->etablissement?->nom }}</span>
                        </td>
                        <td class="px-4 py-3"><span class="inline-flex items-center px-2 py-0.5 bg-amber-50 text-amber-700 rounded text-xs font-medium">{{ $e->services_count }}</span></td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $e->is_active ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-600' }}">
                                <span class="w-1.5 h-1.5 rounded-full {{ $e->is_active ? 'bg-green-500' : 'bg-slate-400' }}"></span>
                                {{ $e->is_active ? 'Actif' : 'Inactif' }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-1">
                                <button wire:click="openEdit({{ $e->id }})" class="p-1.5 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                                <button wire:click="confirmDelete({{ $e->id }})" class="p-1.5 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-12 text-center text-slate-500 text-sm">Aucun étage.</td></tr>
                @endforelse
            </tbody>
        </table>
        @if($etages->hasPages())<div class="px-4 py-3 border-t border-slate-200">{{ $etages->links() }}</div>@endif
    </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-xl p-6">
                    <div class="flex items-center justify-between mb-5">
                        <h3 class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Modifier' : 'Nouvel étage' }}</h3>
                    ...
                    <form wire:submit.prevent="save" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1">Bâtiment <span class="text-red-500">*</span></label>
                            <select wire:model="batiment_id" class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                                <option value="">— Choisir —</option>
                                @foreach($batiments as $b)
                                    <option value="{{ $b->id }}">{{ $b->nom }} ({{ $b->etablissement?->nom }})</option>
                                @endforeach
                            </select>
                            @error('batiment_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Numéro <span class="text-red-500">*</span></label>
                            <input type="number" wire:model="numero" class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="nom" placeholder="Ex: RDC, Mezzanine..." class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                        </div>
                        <div class="md:col-span-2">
                        ...
                        <div class="md:col-span-2">
                            <label class="inline-flex items-center gap-2 cursor-pointer">
                                ...
                                <span class="text-sm text-slate-700">Étage actif</span>
                            </label>
                        </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <div><h3 class="text-lg font-semibold text-slate-900">Confirmer la suppression</h3><p class="text-sm text-slate-500 mt-1">Services, chambres et lits liés seront supprimés.</p></div>
                    </div>
