<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h2 class="text-2xl font-bold text-slate-900">Toutes les admissions</h2>
            <p class="text-slate-500 mt-1">Vue globale — tous établissements confondus.</p>
        </div>
        <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-indigo-50 border border-indigo-200 text-indigo-700 text-xs font-medium rounded-full">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-4">
            ...
            <p class="text-2xl font-bold text-red-700 mt-1">{{ $stats['en_cours'] }}</p>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-4">
            <p class="text-xs text-green-600 font-medium">Terminées</p>
            <p class="text-2xl font-bold text-green-700 mt-1">{{ $stats['terminees'] }}</p>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-4">
            <p class="text-xs text-amber-600 font-medium">Transférées</p>
            <p class="text-2xl font-bold text-amber-700 mt-1">{{ $stats['transferees'] }}</p>
        </div>
    </div>

    <div class="bg-white rounded-xl border border-slate-200 p-4 mb-4 grid grid-cols-1 md:grid-cols-5 gap-3">
        <div class="md:col-span-2 relative">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Rechercher par patient ou n° dossier..." class="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
        </div>
        <select wire:model.live="filterEtablissementId" class="px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
            <option value="">Tous les établissements</option>
            @foreach($etablissements as $e)
                <option value="{{ $e->id }}">{{ $e->nom }} @if($e->code) ({{ $e->code }}) @endif</option>
            @endforeach
        </select>
        <select wire:model.live="filterServiceId" class="px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
            <option value="">Tous les services</option>
            @foreach($services as $s)
                <option value="{{ $s->id }}">{{ $s->nom }}</option>
            @endforeach
        </select>
        <select wire:model.live="filterStatut" class="px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
            ...
            <table class="w-full">
                <thead class="bg-slate-50 border-b border-slate-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase">Service / Lit</th>
                    ...
                    @forelse($admissions as $a)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 text-sm text-slate-600">
                                <p class="font-medium text-slate-700">{{ $a->service?->etage?->batiment?->etablissement?->nom ?? '—' }}</p>
                                <p class="text-xs text-slate-400">{{ $a->service?->etage?->batiment?->etablissement?->code ?? '' }}</p>
                            </td>
                            <td class="px-4 py-3 text-sm text-slate-600">
                                <p class="font-medium text-slate-700">{{ $a->service?->nom }}</p>
                                @if($a->lit)
                                    <p class="text-xs text-slate-400 font-mono">Ch. {{ $a->lit->chambre?->numero }} · Lit {{ $a->lit->numero }}</p>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <button wire:click="voirDetail({{ $a->id }})"
                                        class="px-2.5 py-1 text-xs text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-md font-medium">
                                    Détails
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="7" class="px-4 py-12 text-center text-slate-500 text-sm">Aucune admission trouvée.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
                ...
                <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div class="md:col-span-2 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
                            <p class="text-xs uppercase tracking-wider text-indigo-700 font-semibold mb-1">Patient</p>
                        ...
                        <div class="md:col-span-2">
                            <p class="text-xs text-slate-500 uppercase font-semibold">Établissement</p>
                            <p class="text-slate-900 mt-1 font-medium">{{ $admissionDetail->service?->etage?->batiment?->etablissement?->nom ?? '—' }}</p>
                            <p class="text-xs text-slate-500">{{ $admissionDetail->service?->etage?->batiment?->etablissement?->adresse ?? '' }}</p>
                        </div>
                        <div>
