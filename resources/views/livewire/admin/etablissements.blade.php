<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h2 class="text-2xl font-bold text-slate-900">Établissements</h2>
            <p class="text-slate-500 mt-1">Hôpitaux et centres de soins gérés par le système.</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-4 mb-4">
        <div class="relative">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Rechercher par nom ou code..."
            ...
            <table class="w-full">
                <thead class="bg-slate-50 border-b border-slate-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Adresse</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Contact</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Bâtiments</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($etablissements as $e)
                        <tr class="hover:bg-slate-50">
                            <td class="px-4 py-3"><span class="font-mono text-xs bg-slate-100 px-2 py-1 rounded">{{ $e->code }}</span></td>
                            <td class="px-4 py-3 font-medium text-slate-900">{{ $e->nom }}</td>
                            <td class="px-4 py-3 text-sm text-slate-600">{{ $e->adresse ?: '—' }}</td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <button wire:click="openEdit({{ $e->id }})" class="p-1.5 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <button wire:click="confirmDelete({{ $e->id }})" class="p-1.5 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg">
                    ...
                    @empty
                        <tr><td colspan="7" class="px-4 py-12 text-center text-slate-500 text-sm">Aucun établissement.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($etablissements->hasPages())
            <div class="px-4 py-3 border-t border-slate-200">{{ $etablissements->links() }}</div>
        @endif
    </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6">
                    <div class="flex items-center justify-between mb-5">
                        <h3 class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Modifier' : 'Nouvel établissement' }}</h3>
                    ...
                    <form wire:submit.prevent="save" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="code" placeholder="HER, CHU01..." class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm uppercase">
                        ...
                        <div class="md:col-span-2">
                            <label class="inline-flex items-center gap-2 cursor-pointer">
                                ...
                                <span class="text-sm text-slate-700">Établissement actif</span>
                            </label>
                        </div>
                ...
                <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-500 mt-1">Tous les bâtiments liés seront aussi supprimés.</p>
                        </div>
                    </div>
