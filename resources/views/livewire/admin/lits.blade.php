<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h2 class="text-2xl font-bold text-slate-900">Lits</h2>
            <p class="text-slate-500 mt-1">Gestion de la disponibilité des lits.</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-4 mb-4 grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <select wire:model.live="filterChambre" class="px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
            <option value="">Toutes les chambres</option>
            @foreach($chambres as $c)
                <option value="{{ $c->id }}">Ch. {{ $c->numero }} – {{ $c->service?->nom }}</option>
            @endforeach
        </select>
        <select wire:model.live="filterStatut" class="px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm capitalize">
    ...
    @php
        $statutColors = [
            'libre' => ['bg' => 'bg-green-50', 'text' => 'text-green-700', 'dot' => 'bg-green-500'],
            'occupe' => ['bg' => 'bg-red-50', 'text' => 'text-red-700', 'dot' => 'bg-red-500'],
            'maintenance' => ['bg' => 'bg-amber-50', 'text' => 'text-amber-700', 'dot' => 'bg-amber-500'],
            'reserve' => ['bg' => 'bg-blue-50', 'text' => 'text-blue-700', 'dot' => 'bg-blue-500'],
        ];
    @endphp
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <table class="w-full">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase">Lit n°</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase">Chambre</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($lits as $l)
                    @php $c = $statutColors[$l->statut] ?? $statutColors['libre']; @endphp
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3 font-mono font-semibold text-slate-900">{{ $l->numero }}</td>
                        <td class="px-4 py-3 text-sm text-slate-600">Ch. {{ $l->chambre?->numero }}</td>
                        <td class="px-4 py-3 text-sm text-slate-600">{{ $l->chambre?->service?->nom }}</td>
                        <td class="px-4 py-3 text-xs text-slate-500">Ét. {{ $l->chambre?->service?->etage?->numero }} • {{ $l->chambre?->service?->etage?->batiment?->nom }}</td>
                        <td class="px-4 py-3">
                            <div class="inline-block relative" x-data="{ open: false }">
                                <button @click="open = !open" type="button" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $c['bg'] }} {{ $c['text'] }} capitalize hover:ring-2 hover:ring-slate-200">
                                    <span class="w-1.5 h-1.5 rounded-full {{ $c['dot'] }}"></span>
                                    {{ $l->statut }}
                                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                                <div x-show="open" x-cloak @click.outside="open = false" class="absolute z-10 mt-1 w-36 bg-white border border-slate-200 rounded-lg shadow-lg overflow-hidden">
                                    @foreach($statuts as $s)
                                        <button @click="open = false" wire:click="changeStatut({{ $l->id }}, '{{ $s }}')" class="w-full text-left px-3 py-1.5 text-xs hover:bg-slate-50 capitalize">{{ $s }}</button>
                                    @endforeach
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-1">
                                <button wire:click="openEdit({{ $l->id }})" class="p-1.5 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                                <button wire:click="confirmDelete({{ $l->id }})" class="p-1.5 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-12 text-center text-slate-500 text-sm">Aucun lit.</td></tr>
                @endforelse
            </tbody>
        </table>
        @if($lits->hasPages())<div class="px-4 py-3 border-t border-slate-200">{{ $lits->links() }}</div>@endif
    </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-xl p-6">
                    <div class="flex items-center justify-between mb-5">
                        <h3 class="text-lg font-semibold text-slate-900">{{ $editingId ? 'Modifier' : 'Nouveau lit' }}</h3>
                    ...
                    <form wire:submit.prevent="save" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1">Chambre <span class="text-red-500">*</span></label>
                            <select wire:model="chambre_id" class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                                <option value="">— Choisir —</option>
                                @foreach($chambres as $c)
                                    <option value="{{ $c->id }}">Ch. {{ $c->numero }} – {{ $c->service?->nom }}</option>
                                @endforeach
                            </select>
                            @error('chambre_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Numéro <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="numero" placeholder="L1, A, 01..." class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                            @error('numero') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Statut <span class="text-red-500">*</span></label>
                            <select wire:model="statut" class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm capitalize">
                                @foreach($statuts as $s)<option value="{{ $s }}">{{ ucfirst($s) }}</option>@endforeach
                            </select>
                        </div>
                        <div class="md:col-span-2">
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <div><h3 class="text-lg font-semibold text-slate-900">Confirmer la suppression</h3><p class="text-sm text-slate-500 mt-1">Cette action est irréversible.</p></div>
                    </div>
